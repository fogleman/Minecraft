...
# it's very fast, but the disadvantage is that it can't generate overhangs
# and caves
...
class HeightMap(object):
    ...
    def noise_2d(self, x, y):
        n = (X_FACTOR * x + Y_FACTOR * y + SEED_FACTOR * self.seed) \
            & 2147483647
        n = (n >> 13) ^ n
        n = (n * (n * n * 15731 + 789221) + 1376312589) & 2147483647
        return 1 - n / 1073741824.0
    ...
    def smooth_noise_2d(self, x, y):
        ...
        # *-*
        corners = (
            self.noise_2d(x - 1, y - 1) + self.noise_2d(x + 1, y - 1)
            + self.noise_2d(x - 1, y + 1) + self.noise_2d(x + 1, y + 1)) / 16
        # sides
        # -*-
        # *-*
        # -*-
        sides = (self.noise_2d(x - 1, y) + self.noise_2d(x + 1, y)
                 + self.noise_2d(x, y - 1) + self.noise_2d(x, y + 1)) / 8
    ...
    def perlin_noise_2d(self, x, y):
        ...
        for i in range(0, n):
            ...
            total += self.interpolate_noise_2d(x * frequency,
