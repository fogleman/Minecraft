...
class NoiseGen:
    ...
    def _lerp(self, a, b, z):
        mu2 = (1.0 - math.cos(z * 3.14159)) / 2.0
    ...
    def _noise(self, x, z):
        ...
        rec1 = lerp(s, t, x - floorX)
        rec2 = lerp(u, v, x - floorX)
        rec3 = lerp(rec1, rec2, z - floorZ)
        return rec3

    def getHeight(self, x, z, cx, cz):
        newX = (x + (chunkX * CHUNK_SIZE))
        ...
        for a in range(self.noiseParams.octaves - 1):
            ...
            totalValue += self._noise(
                (float(newX)) * freq / self.noiseParams.smoothness,
                (float(newZ)) * freq / self.noiseParams.smoothness
